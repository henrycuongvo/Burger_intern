...
import { GET_USER_INFO_FAIL, GET_USER_INFO_REQUEST, GET_USER_INFO_SUCCESS, USER_LOGIN_FAIL, USER_LOGIN_REQUEST, USER_LOGIN_SUCCESS, USER_REGISTER_FAIL, USER_REGISTER_REQUEST, USER_REGISTER_SUCCESS } from 'redux/reducers/user.reducer';

function* loginSaga(action) {
    try {
        const { data, callback } = action.payload;
        const result = yield axios.post('http://localhost:4000/login', data);
        if (data.email === 'abc' && data.password === 'abc') {
            yield put({
